...
<section class="uk-section uk-booking bg-white uk-position-relative" style="background:#3f3e3e!important;">
    <div class="uk-container uk-container-small"
        ...
        <div class="uk-text-center uk-card uk-card-default uk-card-body uk-border-rounded">
            ...
            <h2 class="uk-text-bold uk-text-success">Requête envoyée avec succès!</h2>
            <h1 class="uk-h4  uk-margin-remove">Cher / Chère<b> {{$name}}</b>{!!$message!!}</h1>
            <hr class="uk-divider-icon">
            <p><b>Meilleurs vœux</b>
            <br>{{$setting->site_name}}
            </p>
            <div>
                @if($trip)
                <a href="{{ url('page/' . tripurl($trip['uri'])) }}" class="uk-button uk-button-primary">Retourner</a>
                @else
                <a href="{{url('/')}}" class="uk-button uk-button-primary">Accueil</a>
                @endif
            </div>
        </div>
    </div>
